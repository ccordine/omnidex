...
The authoritative nested job → objective → task coordination direction is [CHARMELEON-EMERGENT-ORCHESTRATION-INVARIANTS.md](docs/CHARMELEON-EMERGENT-ORCHESTRATION-INVARIANTS.md). It is code-owned persisted state, never a coordinator model: an LLM may fill only the exact unresolved semantic leaf inside one stage.
The authoritative task-local artifact-graph and adapter-projection direction is [CHARMELEON-CONVERGENT-COGNITIVE-DEVELOPMENT-INVARIANTS.md](docs/CHARMELEON-CONVERGENT-COGNITIVE-DEVELOPMENT-INVARIANTS.md). The tree establishes only the work surface; code derives and persists verified artifact interfaces and relations before dependent leaves are dispatched.
