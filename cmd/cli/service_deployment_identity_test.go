...
func TestServiceDeploymentIdentityUsesExactManagedEnvironment(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, ".env"), []byte("DOCKER_CONTEXT=default\nCOMPOSE_PROJECT_NAME=omni-nxt\nHOST_UID=1000\nHOST_GID=1001\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if identity.DockerContext != "default" || identity.ComposeProject != "omni-nxt" ||
		identity.HostUID != "1000" || identity.HostGID != "1001" || identity.runtimeUser() != "1000:1001" {
		t.Fatalf("deployment identity = %+v", identity)
	}
	if got := strings.Join(identity.composeCommandPrefix(), " "); got != "docker --context default compose -p omni-nxt" {
		t.Fatalf("Compose prefix = %q", got)
	}
	if got := strings.Join(identity.dockerCommandPrefix(), " "); got != "docker --context default" {
...
func TestServiceDeploymentIdentityRejectsMissingBlankDuplicateAndInvalidAuthority(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "missing project", raw: "DOCKER_CONTEXT=default\n", want: "does not define COMPOSE_PROJECT_NAME"},
