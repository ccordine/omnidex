...
func (stations *PortableStations) Resolve(ctx context.Context, call SearchTermsCall) (SearchTermsDecision, error) {
	if err := validatePortableSearchTermsCall(call); err != nil {
		return SearchTermsDecision{}, err
	}
	input := assemblyline.WebSearchTermsInput{
	...
	job, err := assemblyline.NewWebSearchTermsJob(input)
	if err != nil {
		return SearchTermsDecision{}, fmt.Errorf("build web search terms job: %w", err)
	}
	result, err := stations.run(ctx, job)
	if err != nil {
		return SearchTermsDecision{}, err
	}
	decision, err := assemblyline.DecodeWebSearchTermsDecision(input, result.Candidate)
	if finalizeErr := stations.finalize(ctx, job, result, err); finalizeErr != nil {
		return SearchTermsDecision{}, combinePortableStationErrors(err, finalizeErr)
	}
	if err != nil {
		return SearchTermsDecision{}, err
	}
	return SearchTermsDecision{Terms: append([]string{}, decision.Terms...)}, nil
}

func (stations *PortableStations) Select(ctx context.Context, call RelevanceCall) (RelevanceDecision, error) {
	...
	input := assemblyline.WebRelevanceInput{
		ExactQuestion: call.Question,
		Context:       assemblyline.CloneObjectiveContext(call.Context),
		Candidates:    candidates, MaxSelections: call.MaxSelections,
	}
	job, err := assemblyline.NewWebRelevanceJob(input)
	if err != nil {
		return RelevanceDecision{}, fmt.Errorf("build web relevance job: %w", err)
	}
	result, err := stations.run(ctx, job)
	if err != nil {
		return RelevanceDecision{}, err
	}
	decision, err := assemblyline.DecodeWebRelevanceDecision(input, result.Candidate)
	if finalizeErr := stations.finalize(ctx, job, result, err); finalizeErr != nil {
		return RelevanceDecision{}, combinePortableStationErrors(err, finalizeErr)
	}
	if err != nil {
		return RelevanceDecision{}, err
	}
...
func (stations *PortableStations) Synthesize(ctx context.Context, call GroundedSynthesisCall) (GroundedSynthesisDecision, error) {
	...
	input := assemblyline.WebGroundedSynthesisInput{
		ExactQuestion: call.Question,
		Context:       assemblyline.CloneObjectiveContext(call.Context),
		Evidence:      evidence,
		MaxParagraphs: call.MaxParagraphs, MaxParagraphBytes: call.MaxParagraphBytes,
	}
	job, err := assemblyline.NewWebGroundedSynthesisJob(input)
	if err != nil {
		return GroundedSynthesisDecision{}, fmt.Errorf("build web grounded synthesis job: %w", err)
	}
	result, err := stations.run(ctx, job)
	if err != nil {
		return GroundedSynthesisDecision{}, err
	}
	decision, err := assemblyline.DecodeWebGroundedSynthesisDecision(input, result.Candidate)
	if finalizeErr := stations.finalize(ctx, job, result, err); finalizeErr != nil {
		return GroundedSynthesisDecision{}, combinePortableStationErrors(err, finalizeErr)
	}
	if err != nil {
		return GroundedSynthesisDecision{}, err
	}
	...
	return GroundedSynthesisDecision{Paragraphs: paragraphs}, nil
}
