...
var fieldRegistry = []fieldDefinition{
	...
	{Key: "grounded_answer_model", Label: "Grounded answer", Description: "Returns one bounded untrusted paragraph inventory, one question-relevance relation, one factual-support relation, or one source-attribution relation per repository-grounding call", EnvironmentKey: "OMNI_GROUNDED_ANSWER_MODEL", Stations: []station.ID{station.GroundedAnswer}},
	{Key: "database_schema_selection_model", Label: "Database schema selection", Description: "Selects the model for one repeated opaque choice among remaining code-known relation descriptions", EnvironmentKey: "OMNI_DATABASE_SCHEMA_SELECTION_MODEL", Stations: []station.ID{station.DatabaseSchemaSelection}},
	...
	{Key: "coding_fragment_model", Label: "Coding fragment", Description: "Returns one ordinary path-blind implementation body from a bounded local contract and continues the same persisted context for bounded defect correction", EnvironmentKey: "OMNI_CODING_FRAGMENT_MODEL", Stations: []station.ID{station.CodingFragment}},
}
