...
func (r *Repository) ResetDatabase(
	...
	if err := acquireDatabaseSetupLock(ctx, conn); err != nil {
		return errors.Join(err, destroyLockedConnection(conn))
	}
	locked := true
	defer func() {
		if locked {
			resultErr = errors.Join(resultErr, releaseDatabaseSetupLock(conn))
		}
	}()

	runtimeSchema, err := currentDatabaseSetupSchema(ctx, conn)
	if err != nil {
		return err
	}
	...
	if err := executeDatabaseSetup(ctx, conn, body); err != nil {
		return err
	}
	if err := releaseDatabaseSetupLock(conn); err != nil {
		locked = false
		return err
	}
	locked = false
...
func currentDatabaseSetupSchema(ctx context.Context, conn *pgxpool.Conn) (string, error) {
	...
	if err := validateResettableRuntimeSchema(runtimeSchema, true); err != nil {
		return "", err
	}
	return runtimeSchema, nil
}

func executeDatabaseSetup(ctx context.Context, conn *pgxpool.Conn, body []byte) error {
	if len(body) == 0 {
		return fmt.Errorf("execute database setup requires SQL bytes")
	}
...
func resetExactRuntimeSchema(ctx context.Context, conn *pgxpool.Conn) error {
	...
	defer tx.Rollback(context.Background())
	if _, err := tx.Exec(ctx, `SET LOCAL standard_conforming_strings TO on`); err != nil {
		return fmt.Errorf("set runtime schema reset SQL mode: %w", err)
	}
...
func validateResettableRuntimeSchema(schema string, owned bool) error {
	if !owned || schema == "" || schema == "public" || schema == "information_schema" ||
		...
		return fmt.Errorf("database reset refuses non-owned or broad runtime schema %q", schema)
	}
	return nil
}
