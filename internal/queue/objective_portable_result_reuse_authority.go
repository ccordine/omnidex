...
type objectivePortableReuseSameJobAuthority struct {
	JobID int64 `json:"job_id"`
}

// canonicalObjectivePortableReuseAuthority is intentionally minimal for an
// ordinary objective: reuse is restricted to the same job and exact portable
// job, so the immutable job identity is sufficient. Roleplay additionally
// binds the exact fictional turn authority before any prior result can match.
func canonicalObjectivePortableReuseAuthority(job model.Job) ([]byte, bool, error) {
	roleplayJob, err := isRoleplayPortableReuseJob(job)
	if err != nil {
		return nil, false, err
	}
	if roleplayJob {
		authority, err := canonicalRoleplayPortableReuseAuthority(job)
		return authority, true, err
	}
	if job.ID < 1 {
		return nil, false, fmt.Errorf("objective portable reuse job has no exact identity")
	}
	authority, err := exactjson.Canonical(objectivePortableReuseSameJobAuthority{JobID: job.ID})
	if err != nil {
		return nil, false, fmt.Errorf("canonicalize objective portable reuse authority: %w", err)
	}
	return authority, false, nil
}

func isRoleplayPortableReuseJob(job model.Job) (bool, error) {
	...
	if err := json.Unmarshal(metadata, &marker); err != nil {
		return false, fmt.Errorf("decode objective portable reuse job %d metadata: %w", job.ID, err)
	}
	return marker.ChannelMode == model.ChannelModeRoleplay, nil
}

func requireObjectivePortableReuseJobBindingTx(
	ctx context.Context,
	tx pgx.Tx,
	job model.Job,
) (bool, error) {
	...
	return true, requireRoleplayPortableReuseJobBindingTx(ctx, tx, job)
}
