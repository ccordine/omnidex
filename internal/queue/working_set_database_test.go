...
func TestPostgresWorkingSetCommandsMaterializeAndReplayExactly(t *testing.T) {
	...
	created, err := repository.CreateCurrentWorkingSet(ctx, authority, budget)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := repository.CreateCurrentWorkingSet(ctx, authority, budget); !errors.Is(err, ErrWorkingSetExists) {
	...
	for _, command := range commands {
		...
		if len(events) == 1 {
			...
			if _, err := repository.ApplyWorkingSetCommand(ctx, authority, stale); !errors.Is(err, workingset.ErrVersionConflict) {
	...
	replayed, err := repository.ApplyWorkingSetCommand(ctx, authority, commands[0])
...
func TestPostgresWorkingSetRejectsStaleGenerationAfterJobLock(t *testing.T) {
	...
	if _, err := repository.CreateCurrentWorkingSet(ctx, authority, budget); err != nil {
		t.Fatal(err)
	}
	...
	_, err := repository.ApplyWorkingSetCommand(ctx, authority, first)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := repository.ApplyWorkingSetCommand(ctx, authority, stale); !errors.Is(err, ErrStaleStepAttempt) {
		t.Fatalf("stale generation command error=%v, want ErrStaleStepAttempt", err)
	}
	if replayed, err := repository.ApplyWorkingSetCommand(ctx, authority, first); !errors.Is(err, ErrStaleStepAttempt) {
		t.Fatalf("retired generation replay event=%+v error=%v", replayed, err)
	}
	...
	if _, err := repository.CreateCurrentWorkingSet(ctx, authority, budget); !errors.Is(err, ErrStaleStepAttempt) {
		t.Fatalf("stale authority created generation-two set: %v", err)
	}
}
...
func TestPostgresWorkingSetCommandIdentityCannotCrossOwners(t *testing.T) {
	...
	firstAuthority := claimWorkingSetTestJob(t, ctx, repository, firstJob)
	secondAuthority := claimWorkingSetTestJob(t, ctx, repository, secondJob)
	budget := workingset.Budget{MaxItems: 1, MaxBytes: 16}
	for _, authority := range []model.StepAttemptAuthority{firstAuthority, secondAuthority} {
		if _, err := repository.CreateCurrentWorkingSet(ctx, authority, budget); err != nil {
			t.Fatal(err)
		}
	}
	...
	if _, err := repository.ApplyWorkingSetCommand(ctx, firstAuthority, command); err != nil {
		t.Fatal(err)
	}
	if _, err := repository.ApplyWorkingSetCommand(ctx, secondAuthority, command); !errors.Is(err, workingset.ErrCommandIDConflict) {
