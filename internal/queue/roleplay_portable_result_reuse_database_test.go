...
func TestPostgresRoleplayPortableResultReusePreservesFailedAcceptedLeaf(t *testing.T) {
	...
	duplicate, duplicateFound, err := fixture.Repository.ReuseRoleplayPortableResult(t.Context(), request)
	...
	if _, found, err := fixture.Repository.ReuseRoleplayPortableResult(t.Context(),
		RoleplayPortableResultReuseRequest{
			...
			Station: roleplayPortableReuseStation(t, changedWork),
		}); err != nil || found {
...
func TestPostgresRoleplayPortableResultReuseExcludesCompletedTurn(t *testing.T) {
	...
	candidates, listErr := listRoleplayPortableReuseCandidatesTx(
	...
	if _, found, err := fixture.Repository.ReuseRoleplayPortableResult(
		t.Context(), request,
	); err != nil || found {
...
func TestPostgresRoleplayPortableResultReuseExcludesCurrentAttemptAndFailedOutcome(t *testing.T) {
	...
	if _, found, err := fixture.Repository.ReuseRoleplayPortableResult(t.Context(),
		RoleplayPortableResultReuseRequest{
			...
			Station: roleplayPortableReuseStation(t, resolvedRoot),
		}); err != nil || found {
	...
	rejectedOutcome := persistRejectedRoleplayPortableReuseLeaf(
	...
	for label, ineligible := range map[string]assemblyline.PortableJob{
		...
		if _, found, err := fixture.Repository.ReuseRoleplayPortableResult(t.Context(),
			RoleplayPortableResultReuseRequest{
				...
				Station: roleplayPortableReuseStation(t, ineligible),
			}); err != nil || found {
...
func assertRoleplayPortableReuseAbsent(
	...
	if _, found, err := repository.ReuseRoleplayPortableResult(t.Context(),
		RoleplayPortableResultReuseRequest{
