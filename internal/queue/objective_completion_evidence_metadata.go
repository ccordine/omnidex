...
func objectiveCitationMetadataKeyAllowed(key string, web, database, roleplayResearch bool) bool {
	switch key {
	case "capsule_id", "objective_id", "objective_kind", "requirement_id":
		return true
	case "web_evidence_id", "web_evidence_index", "paragraph_indexes", "source_observed_at", "source_truncated":
		return web
	case "source_acquired_at", "database_evidence_id", "database_row_start", "database_row_end":
		return database
...
func validateObjectiveDatabaseMetadata(record evidence.Record) error {
	...
	_, _, _, err = databaseCitationPosition(record.Metadata)
	return err
}
