...
func TestPostgresRoleplayPortableResultReuseAllowsIdenticalDirectAndCorrectedResults(t *testing.T) {
	fixture := newRoleplayPortableReuseDatabaseFixture(t, "roleplay-reuse-identical-paths")
	root := roleplayPortableReuseRootJob(t, "resolve one path-independent leaf")
	...
	correctionClaim, err := fixture.Repository.ClaimNextStep(
		t.Context(), "identical-paths-correction-source",
	)
	...
	request, targetJob := claimRoleplayPortableReuseTarget(
		t, fixture, "identical-paths-target", instruction, root,
	)
	reused, found, err := fixture.Repository.ReuseRoleplayPortableResult(t.Context(), request)
	if err != nil || !found {
	...
	if reused.Result.Candidate != roleplayPortableReuseExactCandidate ||
		...
		reused.Receipt.SourceWorkID != correction.ID ||
		reused.Receipt.SourceGapOutcomeID == direct.Outcome.ID {
		t.Fatalf("identical path reuse did not retain deterministic newest provenance: %+v", reused)
	}

	again, againFound, err := fixture.Repository.ReuseRoleplayPortableResult(t.Context(), request)
	if err != nil || !againFound || again.Receipt.ID != reused.Receipt.ID ||
		again.Receipt.SourceGapOutcomeID != correctionOutcome.ID {
		t.Fatalf("idempotent identical path reuse=%+v found=%t err=%v", again, againFound, err)
	}
	var receipts int
	if err := fixture.Pool.QueryRow(t.Context(), `
		SELECT COUNT(*) FROM roleplay_portable_result_reuses WHERE target_job_id=$1
	...
	if receipts != 1 {
		t.Fatalf("identical path reuse receipts=%d want 1", receipts)
	}
}
