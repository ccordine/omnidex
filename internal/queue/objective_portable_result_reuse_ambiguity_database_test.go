...
func TestPostgresObjectivePortableResultReuseRejectsDivergentExactResults(t *testing.T) {
	...
	if err := fixture.Pool.QueryRow(t.Context(), `
		SELECT COUNT(*) FROM objective_portable_result_reuses WHERE target_job_id=$1
...
func TestPostgresObjectivePortableResultReuseAllowsIdenticalExactResults(t *testing.T) {
	...
	again, againFound, err := fixture.Repository.ReuseObjectivePortableResult(t.Context(), request)
...
func seedRoleplayPortableReuseAcceptedSource(
	...
	outcome := persistObjectivePortableReuseLeaf(
...
func claimRoleplayPortableReuseTarget(
	...
	instruction string,
	root assemblyline.PortableJob,
) (ObjectivePortableResultReuseRequest, model.Job) {
	t.Helper()
	_, job, err := enqueueNarratorRoleplayTurn(
		t.Context(), fixture.Repository, fixture.Channel.ID, instruction,
	)
	...
	return ObjectivePortableResultReuseRequest{
