...
func newGeneratedDeploymentDatabaseFixture(t *testing.T, label string) generatedDeploymentDatabaseFixture {
	return newGeneratedDeploymentDatabaseFixtureAtPrefix(t, label, "145")
}

func newGeneratedDeploymentDatabaseFixtureAtPrefix(
	t *testing.T, label, prefix string,
) generatedDeploymentDatabaseFixture {
	t.Helper()
	pool := openIsolatedMigrationPool(t)
	repository, ctx := New(pool), t.Context()
	if err := repository.EnsureSchema(ctx, loadMigrationBundleThroughPrefix(t, prefix)); err != nil {
		t.Fatal(err)
	}
	location := t.TempDir()
