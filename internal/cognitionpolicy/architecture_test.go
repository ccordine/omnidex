...
func TestPolicyImplementsPureCognitionBoundary(t *testing.T) {
	t.Parallel()
	var _ cognition.Policy = (*Policy)(nil)
}

func TestProductionImportsOnlyRegisteredPolicyDependencies(t *testing.T) {
	t.Parallel()
	...
	for _, entry := range entries {
		...
		for _, forbidden := range []string{
			"policy.client.generate(", "embedding(", "transcript", "raw shell", "fallback", "type modelref",
		} {
			if strings.Contains(source, forbidden) {
				t.Fatalf("%s contains forbidden policy path %q", entry.Name(), forbidden)
			}
		}
	}
}
