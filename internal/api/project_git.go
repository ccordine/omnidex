...
const (
	projectGitChangedFileLimit = 64
	projectGitCommitLimit      = 12
)
...
func (s *Server) loadProjectGitStatus(ctx context.Context, project model.Project, location string) (map[string]any, error) {
	if !projectPathAccessibleLocally(location) {
		if client := s.hostBridgeClient(); client != nil {
			return nil, fmt.Errorf("git status requires the project directory on this machine (host bridge path: %s)", location)
		}
		return nil, fmt.Errorf("project directory is not accessible locally")
	}
	runCtx, cancel := context.WithTimeout(ctx, 12*time.Second)
	defer cancel()
	return collectProjectGitStatus(runCtx, location)
}

func collectProjectGitStatus(ctx context.Context, location string) (map[string]any, error) {
	...
	changedFiles := make([]map[string]any, 0, projectGitChangedFileLimit)
	if porcelain, err := gitOutput(ctx, location, "status", "--porcelain=v1", "-u"); err == nil {
		for _, line := range strings.Split(porcelain, "\n") {
			...
			if len(changedFiles) < projectGitChangedFileLimit {
	...
	commits := make([]map[string]any, 0, projectGitCommitLimit)
	if logOut, err := gitOutput(ctx, location, "log", fmt.Sprintf("-%d", projectGitCommitLimit), "--format=%H|%s|%an|%ar"); err == nil {
