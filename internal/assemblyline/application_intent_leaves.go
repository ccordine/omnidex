...
func BuildApplicationRequirementCoveragePrompt(
	...
	return strings.Join([]string{
		...
		"A derived result is independently testable only when its requirement identifies the semantic relation that determines the result and the observable operands, conditions, and result meaning needed to compute an independent oracle. A quality claim about an output is not its determining rule. An otherwise unstated relation may be made explicit only when the immutable request semantically entails exactly one determining rule and its operands, conditions, and result meaning for that outcome; ambiguity means the relation is missing.",
