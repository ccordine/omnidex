...
func BuildApplicationRequirementCandidateResultRelationPrompt(
	...
	return strings.Join([]string{
		...
		"2. For a derived value, return EXPLICIT_DERIVED_RESULT_RELATION when the candidate names its necessary input or condition and determining rule; otherwise return MISSING_DERIVED_RESULT_RELATION. Named transformations, orderings, digests, comparisons or selections, aggregations, and existing per-item grouping keys are rules; equal key values determine groups. When an actor supplies an expression, formula, or operation, or performs a calculation, the chosen rule and operands are observable runtime inputs; neither must be fixed beforehand. Passively describing an unspecified output as calculated, computed, evaluated, generated, or selected names neither its rule nor inputs. Selected, correct, best, useful, or appropriate alone name no rule.",
		"3. Return NO_DERIVED_RESULT only when no derived value is asserted: an action, control, state transition, event, message, artifact creation/availability, or unchanged supplied data, including when a condition only triggers that behavior. Qualitative adjectives do not create a derived value.",
