...
func NewApplicationRequirementCandidateResultRelationCorrectionJob(
	...
	return newPortableJob(WorkApplicationRequirementCandidateResultRelationCorrection, input)
}
...
func (input ApplicationRequirementCandidateResultRelationCorrectionInput) validate() error {
	...
	if err := ValidatePathFreeModelContext(
		"application requirement result-relation correction request", input.ImmutableRequest,
	); err != nil {
		return err
	}
	...
	return input.Grounding.ValidateCorrectionAuthority(
		input.ImmutableRequest, input.Context, input.CurrentCandidate, input.Defect,
	)
}
...
func DecodeApplicationRequirementCandidateResultRelationCorrectionLeaf(
	...
	if leaf == input.CurrentCandidate {
		return "", fmt.Errorf("application requirement candidate result-relation correction repeated the exact defective value")
	}
	return leaf, nil
}
