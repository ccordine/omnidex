...
func TestStimulusTailwindSmokeAppValidator(t *testing.T) {
	...
	pidFile := filepath.Join(root, "omni-webapp.pid")
	t.Cleanup(func() {
...
func TestLiveOllamaBuildsAndServesStimulusTailwindSmokeApp(t *testing.T) {
	...
	prompt := fmt.Sprintf(
		"Build a smoke test demo web app in %s and serve it at http://127.0.0.1:%d/. Use one single first shell command for the whole job: create the directory, write static index.html, start the server, write the PID file, then curl the served page. Use Tailwind CSS from a CDN and Stimulus JS from a CDN. The page must include visible text Omni Stimulus Tailwind Smoke, Tailwind utility classes, a Stimulus controller with data-controller, and a button wired with data-action. Do not use npm or install packages. Create files in the foreground. The command must start the server before curl. Do not use '&& nohup ... &'. Use this server shape after file creation with semicolons: nohup python3 -m http.server %d --bind 127.0.0.1 --directory %s > %s 2>&1 & server_pid=$!; echo \"$server_pid\" > %s; Then verify with a retry loop like: for i in 1 2 3 4 5; do curl -fsS http://127.0.0.1:%d/ | grep 'Omni Stimulus Tailwind Smoke' && break; sleep 1; done.",
		appDir,
		port,
		port,
		appDir,
