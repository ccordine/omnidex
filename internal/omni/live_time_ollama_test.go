...
func TestLiveOllamaFindsVirginiaTimeFromCommandEvidence(t *testing.T) {
	...
	if !virginiaTimeEvidenceMatches(evidence, baselineHour) {
		t.Fatalf(
			"Omni time evidence did not overlap Virginia baseline hour\nbaseline_hour=%02d\ncommand=%q\nanswer=%q\nstdout=%s\nstderr=%s",
			baselineHour,
			result.Command,
			result.Answer,
			stdout.String(),
...
func TestLiveOllamaFindsPattayaTimeWithoutFalseCapabilityLoop(t *testing.T) {
	...
	if !pattayaTimeEvidenceMatches(evidence, baselineHour) {
		t.Fatalf(
			"Omni Pattaya time evidence did not overlap Thailand baseline hour\nbaseline_hour=%02d\ncommand=%q\nanswer=%q\nstdout=%s\nstderr=%s\nobservations=%#v",
			baselineHour,
			result.Command,
			result.Answer,
			stdout.String(),
...
func assertNoFalseCapabilityLimitation(t *testing.T, client *OllamaClient, result CommandDecisionResult, stdout, stderr string) {
	...
	if containsFalseRealtimeLimitation(evidence) {
		t.Fatalf("live Omni output contains false capability limitation\ncommand=%q\nanswer=%q\nstdout=%s\nstderr=%s\nobservations=%#v",
