...
function SharedValueActionProbe({ state, actions }: FeatureViewProps) {
	...
	return (
		<div>
			...
			<button type="button" onClick={() => actions.append('items', { label: 'next' })}>Append item</button>
			<button type="button" onClick={() => actions.removeAt('items', 0)}>Remove item</button>
			<button type="button" onClick={() => actions.set('invalid', new Date() as unknown as SharedValue)}>Reject invalid</button>
...
describe('shared value boundaries', () => {
	...
	it('keeps action updates immutable and reports rejected values without committing', async () => {
		observedEmptyStates.length = 0;
		...
		fireEvent.click(screen.getByRole('button', { name: 'Append item' }));
		...
		expect(secondItems).toEqual([{ label: 'first' }, { label: 'next' }]);
		...
		const thirdState = feature.application.snapshot()[%[1]s] as FeatureState;
		const thirdItems = thirdState.items as readonly SharedValue[];
		expect(thirdState).not.toBe(secondState);
		expect(thirdItems).not.toBe(secondItems);
		expect(secondItems).toEqual([{ label: 'first' }, { label: 'next' }]);
		expect(thirdItems).toEqual([{ label: 'next' }]);

		const beforeInvalid = feature.application.snapshot();
		const consoleError = vi.spyOn(console, 'error').mockImplementation(() => undefined);
		fireEvent.click(screen.getByRole('button', { name: 'Reject invalid' }));
		await waitFor(() => expect(screen.getByRole('alert')).toHaveTextContent(/Shared value publication/));
		expect(feature.application.snapshot()).toBe(beforeInvalid);
