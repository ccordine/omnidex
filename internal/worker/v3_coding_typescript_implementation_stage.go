...
func (executor *directCodingTypeScriptProjectStageExecutor) closeImplementationBeforeVerification(
	...
	return closeDirectCodingTypeScriptImplementation(
		stage, ref.Block.TaskID, ref.Block.ID, source,
		func(
			projection *directCodingProgram,
			validators ...func(*directCodingProgram) error,
		) error {
			return executor.session.stageTypeScriptProgramIn(
				executor.workspace.Root(), projection, [][]string{{"run", "typecheck"}},
				executor.progress, validators...,
			)
		},
	)
}
...
func projectDirectCodingTypeScriptImplementationStage(
	...
	for blockID, source := range stage.Generated {
		if _, exists := retained[blockID]; exists && strings.TrimSpace(source) != "" {
			projection.Generated[blockID] = source
		}
	}
