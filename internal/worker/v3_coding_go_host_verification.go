...
func (session *directCodingSession) verifyAuthoritativeGoWorkspace(program directCodingProgram, assembly directCodingAssembly) (resultErr error) {
	...
	workspace, err := openDirectCodingGoStageWorkspace(session, program, queue.VerificationHostInstall)
	if err != nil {
		return err
	}
	defer func() {
	...
	return workspace.verifyFinal(&program, queue.VerificationHostFinal)
}
