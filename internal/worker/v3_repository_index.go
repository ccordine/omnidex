...
func (r *nativeRuntimeV3) captureExistingRepositoryIndexWithAnalysis(
	root string,
	adapterID string,
) (repositoryindex.Result, error) {
	result, err := r.captureExistingRepositoryIndex(root)
	if err != nil {
		return result, err
	}
	analysis, err := r.demandExistingRepositoryAnalysis(result.Snapshot, adapterID)
	if err != nil {
		return result, err
	}
...
func (session *directCodingSession) requireExistingRepositoryAnalysis(
	adapterID string,
) (repositoryfacts.Analysis, error) {
	if session == nil || session.runtime == nil || session.repositoryIndex == nil {
		return repositoryfacts.Analysis{}, fmt.Errorf(
			"repository analysis demand requires one indexed coding session",
		)
	}
	for _, analysis := range session.repositoryIndex.Analyses {
		if analysis.Adapter.Name != adapterID {
			continue
		}
		if err := analysis.Validate(session.repositoryIndex.Snapshot); err != nil {
			return repositoryfacts.Analysis{}, fmt.Errorf(
				"cached repository analysis %s: %w", adapterID, err,
			)
		}
		if !analysis.Complete {
			return repositoryfacts.Analysis{}, fmt.Errorf(
				"cached repository analysis %s is incomplete", adapterID,
			)
		}
		return analysis, nil
	}
	analysis, err := session.runtime.demandExistingRepositoryAnalysis(
		session.repositoryIndex.Snapshot, adapterID,
	)
	if err != nil {
		return repositoryfacts.Analysis{}, err
	}
	session.repositoryIndex.Analyses = append(
		session.repositoryIndex.Analyses, analysis,
	)
	return analysis, nil
}
