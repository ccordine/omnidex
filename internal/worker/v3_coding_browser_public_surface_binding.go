...
func (executor *directCodingTypeScriptProjectStageExecutor) validateTaskBrowserPublicSurface(
	...
	if err := validateDirectCodingBrowserFrozenPublicSurface(
		taskID, binding, current, verificationID, currentResultRelation.Receipt,
	); err != nil {
		return err
	}
	return nil
...
func validateDirectCodingBrowserFrozenPublicSurface(
	taskID string,
	binding directCodingBrowserPublicSurfaceBinding,
	current directCodingBrowserPublicSurfaceBinding,
	...
	if current.implementationBlockID != binding.implementationBlockID ||
		...
		resultRelation != binding.resultRelation ||
