...
func TestDBManagerScansSchemaAndRunsLLMGeneratedMemoryQuery(t *testing.T) {
	...
	runner.queryResults["SELECT mc.id, mc.agent_id, mc.kind, mc.content FROM memory_chunks mc WHERE mc.content ILIKE '%laravel%' ORDER BY mc.created_at DESC LIMIT 5"] = []MemorySQLRow{
		{"id": int64(7), "agent_id": "doc_manager", "kind": "documentation_research", "content": "Laravel required validation rule: field must be present and not empty."},
	}
	client := &fakeCommandDecisionClient{responses: []string{
		`{"sql":"SELECT mc.id, mc.agent_id, mc.kind, mc.content FROM memory_chunks mc WHERE mc.content ILIKE '%laravel%' ORDER BY mc.created_at DESC LIMIT 5","answer":"Search memory chunks for Laravel documentation."}`,
	}}

	result, err := RunDBManagerQuery(context.Background(), "Find saved Laravel validation documentation memories.", runner, client)
	if err != nil {
		t.Fatal(err)
	}
	if client.calls != 1 {
	...
	if len(result.Rows) != 1 || !strings.Contains(stringFromAny(result.Rows[0]["content"]), "Laravel required validation") {
...
func TestDBManagerCanPointAtDifferentPostgresSchema(t *testing.T) {
	...
	runner.queryResults[sql] = []MemorySQLRow{
		{"name": "billing-console", "framework": "Laravel", "body": "Dashboard pattern uses dense tables and restrained controls."},
	}
	client := &fakeCommandDecisionClient{responses: []string{
