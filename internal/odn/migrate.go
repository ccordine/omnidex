...
func DefaultMigrationDBConfig() MigrationDBConfig {
	cfg := MigrationDBConfig{
		Mode:      envOrDefault("ODN_DB_MODE", "docker_exec"),
		Container: envOrDefault("ODN_PG_CONTAINER", "postgres_db"),
		Host:      envOrDefault("ODN_DB_HOST", "127.0.0.1"),
		Port:      envOrDefault("ODN_DB_PORT", "5432"),
		Database:  envOrDefault("ODN_DB_NAME", "laravel"),
		User:      envOrDefault("ODN_DB_USER", "laraveluser"),
		Password:  os.Getenv("ODN_DB_PASSWORD"),
		SSLMode:   envOrDefault("ODN_DB_SSLMODE", "disable"),
	}
	if cfg.Password == "" {
...
func RunMigrateUp(migrationsDir string, cfg MigrationDBConfig, steps int) (string, error) {
	...
	for _, migration := range migrations {
		...
		runSQL := strings.Join([]string{
			"BEGIN;",
			statement,
			fmt.Sprintf("INSERT INTO odn_migrations (version, name, batch, checksum_sha256, applied_at) VALUES ('%s', '%s', %d, '%s', NOW());", migration.Version, migration.Name, batch, migration.Checksum),
			"COMMIT;",
		}, "\n")
...
func RunMigrateDown(migrationsDir string, cfg MigrationDBConfig, steps int) (string, error) {
	...
	for _, row := range versions {
		...
		runSQL := strings.Join([]string{
			"BEGIN;",
			statement,
			fmt.Sprintf("DELETE FROM odn_migrations WHERE version = '%s';", migration.Version),
			"COMMIT;",
		}, "\n")
		if err := exec.ExecSQL(runSQL); err != nil {
...
func (p PsqlExecutor) EnsureMigrationTable() error {
	sql := `
CREATE TABLE IF NOT EXISTS odn_migrations (
...
func (p PsqlExecutor) AppliedMigrations() ([]AppliedMigration, error) {
	raw, err := p.QuerySQL("SELECT version, batch, checksum_sha256 FROM odn_migrations ORDER BY version;")
	if err != nil {
		return nil, err
	}
...
func (p PsqlExecutor) NextBatch() (int, error) {
	raw, err := p.QuerySQL("SELECT COALESCE(MAX(batch), 0) FROM odn_migrations;")
	if err != nil {
		return 0, err
	}
