...
func TestResearchWebDocsParsesAndSearchesLocalDocs(t *testing.T) {
	...
	result, err := ResearchWebDocs(
		context.Background(),
		"Find Tailwind width and Laravel required validation rule details.",
		[]WebDocSource{
			{Name: "tailwind", URL: server.URL + "/tailwind"},
			{Name: "laravel", URL: server.URL + "/laravel"},
		},
		[]string{
	...
	if !webDocHitsContain(result.Hits, "laravel", "field under validation must be present and not empty") {
		t.Fatalf("missing Laravel validation hit: %#v", result.Hits)
	}
...
func TestLiveWebDocResearchTailwindAndLaravel(t *testing.T) {
	if testing.Short() {
	...
	result, err := ResearchWebDocs(
		context.Background(),
		"Find exact documentation snippets for Tailwind width utilities and Laravel required validation.",
		[]WebDocSource{
			{Name: "tailwind-width", URL: "https://tailwindcss.com/docs/width"},
			{Name: "laravel-validation", URL: "https://laravel.com/docs/12.x/validation"},
		},
		[]string{
			"Utilities for setting the width of an element",
	...
	if !webDocHitsContain(result.Hits, "laravel-validation", "field under validation must be present") {
		t.Fatalf("missing live Laravel validation documentation hit: %#v", result.Hits)
	}
}
