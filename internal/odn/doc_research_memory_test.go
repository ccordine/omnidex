...
<p>Use validation memories only as recalled documentation evidence, not as a fresh prompt.</p>
	...
	question := "Catalog Tailwind width utilities and Laravel required validation documentation for future project work."
	result, err := ResearchWebDocsToMemory(
		ctx,
		question,
		[]WebDocSource{
			{Name: "tailwind-width", URL: server.URL + "/tailwind/width"},
			{Name: "laravel-validation", URL: server.URL + "/laravel/validation"},
		},
		[]string{
		...
		DocResearchMemoryConfig{
			AgentID: "doc_manager",
			Tags:    []string{"tailwind", "laravel", "project-patterns"},
		},
	)
	if err != nil {
		t.Fatal(err)
	...
	for _, wantTag := range []string{"documentation", "doc-research", "tailwind", "laravel", "project-patterns", "source:tailwind-width", "source:laravel-validation"} {
