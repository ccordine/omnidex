...
BEGIN
    ...
    IF observed_source IS NULL OR observed_language <> 'sql' OR
       observed_volatility <> 'i' OR NOT observed_strict THEN
        RAISE EXCEPTION
            'cannot install repository grounding stations: exact prior station function is missing or has different authority';
    END IF;
    observed_sha256 := encode(digest(convert_to(observed_source,'UTF8'),'sha256'),'hex');
    IF observed_sha256 <> expected_pre_sha256 THEN
        RAISE EXCEPTION
            'cannot install repository grounding stations: prior station function hash % differs from %',
...
DECLARE
    observed_source TEXT;
    observed_sha256 TEXT;
    expected_post_sha256 CONSTANT TEXT := '06b8361909b8592e5991e4cd211162543db3697288958d3454cac937fb8fcae9';
BEGIN
    ...
    IF observed_sha256 <> expected_post_sha256 OR
       NOT station_owns_portable_work(
           'repository_evidence_relevance','repository_evidence_relevance','{}'::jsonb
       ) OR NOT station_owns_portable_work(
           'repository_grounded_review','repository_grounded_review','{}'::jsonb
       ) OR NOT station_owns_portable_work(
           'repository_grounded_correction','repository_grounded_correction','{}'::jsonb
       ) OR station_owns_portable_work(
           'repository_grounded_review','repository_evidence_relevance','{}'::jsonb
       ) OR station_owns_portable_work(
           'repository_grounded_correction','repository_grounded_review','{}'::jsonb
       ) OR NOT station_owns_portable_work(
           'conversation_context_selection','conversation_context_selection','{}'::jsonb
       ) OR station_owns_portable_work(
           'repository_evidence_relevance','unsupported_work','{}'::jsonb
       ) THEN
        RAISE EXCEPTION
            'repository grounding station postcondition failed: function hash % differs from % or routing is invalid',
