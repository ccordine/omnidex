...
DECLARE
    ...
    expected_pre_sha256 CONSTANT TEXT := '8f8a24a0e7be864a7d4ce2925d4fe45f8c5de29f35b57089be404354d49c2732';
BEGIN
    ...
    IF observed_source IS NULL OR observed_language <> 'sql' OR
       observed_volatility <> 'i' OR NOT observed_strict THEN
        RAISE EXCEPTION
            'cannot install web claim-evidence review station: exact prior station function is missing or has different authority';
    END IF;
    observed_sha256 := encode(digest(convert_to(observed_source,'UTF8'),'sha256'),'hex');
    IF observed_sha256 <> expected_pre_sha256 THEN
        RAISE EXCEPTION
            'cannot install web claim-evidence review station: prior station function hash % differs from %',
...
DECLARE
    observed_source TEXT;
    observed_sha256 TEXT;
    expected_post_sha256 CONSTANT TEXT := '8f55bfb4027623220c443013e3f14acb8ec8b01d56f108746fa0326600cf265d';
BEGIN
    ...
    IF observed_sha256 <> expected_post_sha256 OR
       NOT station_owns_portable_work(
           ...
           'web_grounded_synthesis_correction','web_grounded_synthesis_correction','{}'::jsonb
       ) OR station_owns_portable_work(
           'web_grounded_synthesis','web_grounded_synthesis_correction','{}'::jsonb
       ) OR station_owns_portable_work(
           'web_grounded_synthesis','web_claim_evidence_review','{}'::jsonb
       ) OR NOT station_owns_portable_work(
           'web_grounded_synthesis','web_grounded_synthesis','{}'::jsonb
       ) OR station_owns_portable_work(
           'web_claim_evidence_review','unsupported_work','{}'::jsonb
       ) THEN
        RAISE EXCEPTION
            'web claim-evidence review station postcondition failed: function hash % differs from % or routing is invalid',
