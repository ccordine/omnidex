...
BEGIN
    ...
    IF station_source IS NULL OR station_language<>'sql' OR
       ...
       'e08e8fe89efa49d540d56769bde6a708cce6945c371236877bde3eab472aac24' THEN
        RAISE EXCEPTION
            'artifact semantic relation split requires the exact migration 177 station function';
    END IF;
END;
$precondition$;

DO $legacy_opening_precondition$
BEGIN
    IF EXISTS (
        SELECT 1
        ...
        WHERE outcome.id IS NULL
          ...
          AND (
              opening.work_kind='known_artifact_truth' OR
              opening.station='coding_known_artifact_truth'
          )
    ) THEN
        RAISE EXCEPTION
            'artifact semantic relation split requires a fresh reset: incompatible active legacy opening exists';
    END IF;
END;
$legacy_opening_precondition$;
...
BEGIN
    ...
    IF station_source IS NULL OR station_language<>'sql' OR
       station_volatility<>'i' OR NOT station_strict OR
       encode(digest(convert_to(station_source,'UTF8'),'sha256'),'hex')<>
       '3d92903f413de33e2c35ad35217754e3d5ae5089e1d5e01c7069c6fded2eea47' OR EXISTS (
        SELECT 1
        FROM (VALUES
        ...
        'coding_repository_artifact_absence',
        'plain_text_artifact_creation','{}'::jsonb
    ) IS DISTINCT FROM FALSE OR station_owns_portable_work(
        'coding_plain_text_artifact_creation',
        'repository_artifact_absence','{}'::jsonb
    ) IS DISTINCT FROM FALSE OR station_owns_portable_work(
        'coding_known_artifact_truth',
        'known_artifact_truth','{}'::jsonb
        ...
        RAISE EXCEPTION
            'artifact semantic relation split postcondition failed';
    END IF;
END;
$postcondition$;
