...
BEGIN
    ...
    IF observed_source IS NULL OR observed_language <> 'sql' OR
       ...
       observed_sha256 <> expected_pre_sha256 THEN
        RAISE EXCEPTION
            'cannot install service endpoint contract station: prior station authority differs';
    END IF;
END $$;
...
BEGIN
    ...
    IF observed_sha256 <> expected_post_sha256 OR EXISTS (
        ...
        RAISE EXCEPTION
            'service endpoint contract station postcondition differs from exact authority';
    END IF;
END $$;
