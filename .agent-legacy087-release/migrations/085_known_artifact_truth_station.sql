...
BEGIN
    ...
    IF observed_source IS NULL OR observed_language <> 'sql' OR
       observed_volatility <> 'i' OR NOT observed_strict THEN
        RAISE EXCEPTION
            'cannot install known artifact truth station: exact prior station function is missing or has different authority';
    END IF;
    observed_sha256 := encode(digest(convert_to(observed_source,'UTF8'),'sha256'),'hex');
    IF observed_sha256 <> expected_pre_sha256 THEN
        RAISE EXCEPTION
            'cannot install known artifact truth station: prior station function hash % differs from %',
...
BEGIN
    ...
    IF observed_sha256 <> expected_post_sha256 OR
       station_owns_portable_work(
        ...
        RAISE EXCEPTION
            'known artifact truth station postcondition failed: function hash % differs from % or routing is invalid',
