...
## 1) Objective
Build a Go-based coding agent platform where:
- LLMs are specialized into narrowly scoped roles.
- Orchestration is deterministic and state-driven.
- Tool selection is argument-less at the router stage (CSV only).
...
- The system can scale to many delegated specialists without becoming hard to debug or maintain.
...
- Use PostgreSQL (existing container and cosine/vector setup already available at `/home/gryph/Projects/container-pgsql`).
- Do not rely on a single general model for whole-task completion.
- Router LLM output must be a comma-separated tool list only.
- Tool LLMs are specialized and should operate with tightly bounded context.
...
### 4.1 Ollama API behavior to design around
...
### 4.2 PostgreSQL behavior to design around
...
### 4.3 Go observability/testing facts to use
