...
7. The semantic front door freezes the accepted requirement set, including an empty set, for one iteration. Only after at least one requirement survives does the first stack consumer resolve and freeze the typed browser, command-line, or service surface; the first product consumer likewise resolves product identity only then. A surface is not a language or framework choice. A candidate beyond that bounded iteration is deferred or discarded as non-authoritative intake rather than becoming a completion blocker. Code fails only when it can prove that a requirement already admitted to the frozen objective cannot fit a required hard capability boundary without producing an incorrect result. A surface without a complete registered project stack fails at stack selection.
