...
14. A mapped source failure opens a correction job for the smallest current-task owner. It receives the current declaration or code-owned local repair region, exact signature, direct capabilities, one code-owned repair imperative, and a bounded path-free diagnostic. Code alone applies a regional replacement and reparses the whole declaration. Other accepted task declarations survive. Correction continues while parser/compiler/test evidence advances and stops on success, no-op, repeated state or cycle, lost authority, provider failure, cancellation, or a real resource limit; an arbitrary attempt count is not a correctness boundary.
