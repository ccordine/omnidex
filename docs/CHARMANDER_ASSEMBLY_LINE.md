...
* derive one concise product/domain identity, excluding requirements, from one immutable request plus source-backed context facts;
* classify whether another task-local runtime implementation requirement remains, or return one such requirement in a separate call;
...
5. The surface station returns one registered delivery surface. A separate product-context station returns one raw product/domain identity leaf from the immutable request and formalized context. Code then owns the bounded task-local runtime requirement fixed point: a raw coverage call returns `REQUIREMENT_REMAINS` or `NO_UNCOVERED_REQUIREMENT`; only the former permits a separate raw call for one source-owning runtime requirement. Surface, technical/structural format, generic test/build, and deployment constraints remain outside this fixed point and are consumed from the immutable request by their narrow owners. Code classifies each candidate as task-local or non-runtime and as one or multiple runtime outcomes. A task-local one-outcome candidate then enters one separate three-way semantic relation: it needs no derived result, states an independently computable determining relation, or omits that relation. Its input carries the complete validated candidate-kind and one-outcome-cardinality receipts; its output is bound to the candidate hash and to hashes of both exact receipts, and code verifies all three identities before using the returned relation. Only an omitted relation permits one exact one-leaf correction using the immutable generation authority; code preserves the decoded candidate, then reruns ordinary kind, cardinality, and result-relation validation, and a second omission fails. This intake station sees no implementation, public surface, verifier, or expected value and cannot accept work or route the workflow. Code decodes, validates, and retains each leaf, repeats coverage up to the ten-requirement bound, and assembles and freezes the typed application intent after it interprets the no-uncovered relation. It never asks a model to emit or review the aggregate intent. Semantic statements may faithfully paraphrase the request: substring, interval, source-order, disjointness, punctuation, and overlap tests are not authority boundaries.
