...
6. For each accepted requirement, one job-specification station receives the typed surface, exact product quote, all accepted requirement quotes, and one focused requirement. It returns only one concrete objective, one through four required behaviors, and one through four observable acceptance criteria. A separate bounded review sees the same user authority plus that derived candidate and may accept it or name exactly one derived field for repair; it cannot contribute replacement prose or new requirements. Code applies any one-field repair to retained state, requires a subsequent review acceptance, assigns task identity and source order, and freezes the complete workload under one content hash. Models cannot return task identity, dependencies, order, tools, paths, completion state, or a whole plan. Derived build decisions are labeled separately from exact user authority and may not invent capabilities, quantities, timing, defaults, or constraints absent from that authority.
