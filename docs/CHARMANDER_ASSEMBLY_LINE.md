...
15. A generated-source model receives one exact parser-qualified source-dialect label, one exact signature, that minimum task projection, an optional independently promoted active procedure, direct symbol declarations, and explicitly permitted symbols. It returns untrusted text from which code projects exactly one matching declaration. Each source language uses its registered parser and scope/authority policy before its composer constructs the complete file. Path-bearing literals and comments fail at the final provenance-aware source boundary; PHP additionally rejects heredoc and nowdoc authority. The TypeScript generation renderer and the one authoritative correction renderer state that exact source grammar before inference; inert separator punctuation is constructed from its numeric character code instead of being embedded in a path-shaped literal. Raw provider responses remain evidence; no language grants a model whole-file authority.
