...
17. TypeScript and registered PHP source failures may enter the same separated repair boundary. Guidance receives the exact language/dialect, signature, current declaration or TypeScript-local region, direct capabilities, compiler-proven lexical scope when available, and one bounded path-free diagnostic; it returns one imperative instruction and no source. The separate executor receives only that instruction and exact mutable source and returns one replacement node. Code validates the original signature and capability boundary, changes only the diagnosed generated block, preserves every other accepted block, and reruns the exact failed stage. PHP acceptance failures route mechanically from the immutable verification block to its single generated implementation owner; generated verification source never becomes repair-model context. Correction stops on success, no-op, repeated instruction or source state, lost authority, provider failure, cancellation, or a code-owned correction limit. Go, JavaScript, Rust, and Java retain no production correction path; an invalid block or failed stage fails explicitly after its single bounded generation call.
