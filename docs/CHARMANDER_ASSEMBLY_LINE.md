...
6. For each accepted requirement, one job-specification station receives the typed surface, exact product quote, all accepted requirement quotes, and one focused requirement. It returns only one concrete objective, one through four required behaviors, and one through four observable acceptance criteria. A separately routed review model sees the same user authority plus that derived candidate and may accept it or name exactly one derived field for repair; it cannot contribute replacement prose or new requirements. Code applies each one-field repair to retained state, requires another independent review, and preserves every new canonical specification state. Review and repair have no numeric correctness limit: they continue while states are new and stop only on acceptance, an exact no-op or repeated state/cycle, lost authority, provider failure, cancellation, or a real resource limit. Code then assigns task identity and source order and freezes the complete workload under one content hash. Models cannot return task identity, dependencies, order, tools, paths, completion state, or a whole plan. Derived build decisions are labeled separately from exact user authority and may not invent capabilities, quantities, timing, defaults, or constraints absent from that authority.
