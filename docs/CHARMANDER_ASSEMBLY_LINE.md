...
Charmander names the historical build that established this coding foundation. The current build is Charmeleon; neither codename defines a subsystem, runtime, framework, worker, or product.
...
* opaque identities, requirements, capability IDs, versions, and lifecycle state;
...
* PostgreSQL persistence for current-service job state, accepted work, and execution evidence.

Every current station uses one raw-text provider transport. Code renders the prompt
and records the actual call under the current job; there is no response-schema field,
structured-output option, or separate projection registry. Code parses and validates
...
* interpret the request's technical-format constraint through one opaque choice among
  registered formats, an unconstrained observation, and an unsupported observation;
...
3. Code bootstraps application context before requirement intake. It records the current workspace state as a typed fact. Repository manifests, indexes, symbols, tests, runtime probes, and external evidence are code-owned acquisition results with their actual observations and source references; models never receive an operation catalog or choose an acquisition mechanism.
...
5. Exactly one bounded requirement-inventory call returns either
   ...
   Code associates each result-relation call with its candidate and consumes the returned
   semantic value directly. An omitted relation opens one separate request-and-context grounding
...
6. Code assigns requirement identities inside the current job and generation. It retains the raw request separately from the path-redacted semantic request; only the redacted request is model-visible. Accepted statements and their semantic results are retained together. Code checks the actual task and requirement values when constructing and projecting verification work; there are no request, candidate, or semantic-receipt hash gates. There is no one-shot quote gate, compatibility route, fallback requirements implementation, or mandatory semantic-review loop.
...
16. Acceptance is stack-specific while task ownership remains neutral. Behavioral acceptance requires a separate owned verification declaration with failure-capable assertions grounded in the owned implementation's observable result. TypeScript, Go, JavaScript, Rust, and Java currently construct those declarations. Compiler success alone is not behavioral acceptance. There is no acceptance-grounding model or reviewer. The upstream requirement sieve cannot retain a derived-result leaf until its candidate's result relation establishes that the leaf either needs no derived oracle or names the semantic relation and observable operands, conditions, and result meaning required to compute one. An underdetermined leaf first receives a separate request-and-validated-context-bound recoverability relation: a negative result discards only that leaf, while only a positive receipt permits correction of that exact leaf and ordinary revalidation before it can become task authority.
   ...
   The implementation cannot escape that declared surface through runtime host authority. All unbound runtime identifiers fail unless present in the exact deterministic ECMAScript/React allowlist. Raw browser-global, DOM-selection, navigation, network, storage, audio, scheduler, dynamic-evaluation, and reflection authority remains unavailable. There is no registered host-capability wrapper, bridge, driver, or runtime-necessity station. Dynamic import and host metadata fail. Computed properties are limited to literals or lexically resolved immutable numeric indexes, while unresolved computed access and reflective destructuring fail. Code statically resolves inline, named, and `useCallback` JSX event handlers; computed event access, aliases, mutation, and event-object escape also fail. The only event-root data path is a direct, read-only `value` or `checked` leaf through `event.target` or `event.currentTarget`, represented by the canonical `event.target.value` and `event.currentTarget.checked` forms. Ordinary numerically indexed domain data remains available without opening dynamic property or DOM authority.
   ...
   An exact derived text result is selectable only as the singular implicit `status` role with its receipt-proven accessible name and qualifies only through a non-negated `toHaveTextContent` assertion using an anchored literal regular expression. Generic text queries and mere presence cannot stand in for that owner or result. An explicit derived-result relation requires this exact named status-output assertion after at least one receipt-grounded public interaction, and the assertion must occur after the final `fireEvent`. A future no-interaction exception would require a separate exact semantic receipt proving that no public operand or operation is needed; the current result-relation receipt does not carry that authority, so zero-interaction explicit-result verification fails loudly. Every other interacting verification still needs a qualifying outcome after its final event; when no named output owns the outcome, the bounded alternatives are compatible checked, validity, value, or display-value control-state assertions with static scalar expectations. Action labels remain public claims that can select only among alternatives already established by the requirement; they never supply the relation or expected value. The static harness alone renders the feature and runs the accepted verification. A generated verifier's receipt-grounding or grammar rejection, and any staged execution failure originating in that generated verification block, is terminal evidence and cannot become implementation-repair model context. Go uses `testing`. JavaScript uses an owned Node test registration and a normalized observation callback; its separate ordinary-text test-body call sees only the accepted requirement, input/result protocol, and assertion authority, never implementation source or declarations. Code requires an executed observation and direct strict assertions against independent expected values. Rust owns a native test registration per task module and a separate path-blind verification body using an observation function, individually declared runtime types, and native `assert_eq!` comparisons. Code parses the operands to establish execution and independent expectations; Cargo runs the actual tests. Java owns a test class and assertion-enabled runner per task. Its separate test-body call receives only the requirement and normalized observation declaration; code validates literal inputs and independent native value comparisons, then executes the exact class with `java -ea`. Disabled assertions fail explicitly. Every registered source executor runs its current task-local checks before beginning the next task; only behavioral assertions can establish the required outcome.
17. When deterministic parsing, signature assembly, scope checking, authority checking, or task-local candidate validation proves one specific defect in a returned body, code must also prove the exact mutable byte span before inference can continue. Code persists the rejected body as evidence and retains it as the splice base, but adds neither that complete body nor surrounding accepted source to the correction prompt. The same persisted fragment-generation call lineage continues with the same immutable model route and retained model context. Its new prompt is one necessary semantic question plus only the exact defective span, and its ordinary plain-text result is replacement text for that span. Code compares the current source with the retained base and validates the byte range, performs the exact splice, and repeats its declaration assembly and validators. At most three total body attempts are permitted. Acceptance retains the code-assembled declaration and releases the continuation; exhaustion fails only that job and releases only its context. There is no preservation instruction, repair plan, replacement schema, control label, response packet, alternate work kind, task restart, or model swap. Every accepted byte outside the span, every accepted block, and every unrelated job remains untouched and runnable.

    The Ollama adapter carries the exact immediate parent's returned native `context`
    token IDs in the correction request. Code restores them from the persisted raw
    response, and PostgreSQL rejects a child whose context differs from that parent.
    The native context stays separate from the new semantic prompt; it is never
    reconstructed from source or another call's history. Token counts and encoded
    request bytes remain bounded, with `truncate=false` and `shift=false`.
    Missing or malformed retained context fails at the first correction consumer;
    accepted results do not require an unused continuation capability.
    Ollama's [generate API](https://github.com/ollama/ollama/blob/main/api/types.go)
    currently supports this context field, although it is deprecated. A provider
    that cannot continue it cannot perform a source correction through this adapter.

    Portable work contains only its registered kind and exact code-owned inputs. Its
    result contains only the semantic candidate. Neither carries a content-derived job ID.
    Recovery compares the retained initial input values within the current job generation
    and step; PostgreSQL call IDs link corrections to their original call. Only the initial
    call stores those inputs. Child calls retain the parent reference and their exact mutable
    span. Recovery neither rehashes a request nor reconstructs a model-owned identity.
18. After every task passes independently, code composes all documents and runs the selected stack's final isolated verification. TypeScript runs integrity-locked npm installation, tests, typechecking, browser smoke coverage, and a Vite production build with the pinned Tailwind CSS v4 plugin; the build must emit utilities used by assembled source. Go runs focused tests, full tests, vet, and build. JavaScript syntax-checks every module, loads the implementation graph, and runs the exact task-owned Node test files. Rust runs locked offline Cargo library checks, task-owned tests, and a complete build. Java uses strict `javac --release` compilation, executes every owned test class with `java -ea`, and creates the application archive. Compiler success is not an independent outcome oracle.
...
20. Only an isolated complete assembly that passes is written to the authoritative workspace. Code repeats leaf validation at the write gate, reconciles exact files and protected paths, reruns the selected stack's authoritative verification, and reaches its code-owned terminal result. A completion claim must establish the accepted behavior, not merely successful commands. The workload has no learned-skill mutation authority.

## Semantic leaf failure and narrow replacement boundaries

A decoded semantic leaf is retained by code and never becomes workflow authority. Code advances after a valid leaf without a ceremonial review. An invalid product-context, requirement-inventory, candidate-relation, surface or selection leaf fails at its owning station; there is no generic response-correction work kind, correction model, retry budget, rejected-response history, or aggregate reconstruction path.
...
## Learned domain procedures

No learned-skill registry or procedure-enrichment workflow is registered in the
current build. Source generation receives the accepted task requirement directly.
Do not describe the deleted bootstrap, sealed replay, or promotion machinery as an
available feature, and do not restore it merely because an old design named it.

The architectural boundary for future support is unchanged: reusable domain
procedures must be learned data in the current-service PostgreSQL lifecycle, not
Go branches, checked-in domain prompts, static skills, or product-specific adapters.
Code must own retrieval, identity, bounds, executable validation, and activation.
A model may supply only one necessary semantic field.

A future implementation must demonstrate actual execution of its validation and
reuse cases before claiming an accepted procedure works. Caller-declared success,
fixture-set hashes, sealed receipts, or a model's approval cannot supply that proof.
This missing capability is not permission to add a domain procedure to make an
observed benchmark pass.
...
The current generic greenfield compiler registers TypeScript/React browser applications and Go, JavaScript, Rust, and Java command-line applications. There is no registered PHP/NGINX/Docker HTTP stack, service-state compiler, or general existing-project mutation workflow. Leaf recognition or plain-text composition alone does not establish a complete application workflow. Browser host operations outside the registered source boundary are unavailable. The current framework does not provide arbitrary dependency installation or cross-machine scheduling. JavaScript, Rust, and Java have task-owned behavioral test bodies and recorded Node/Cargo/JVM execution, but no autonomous build or live-model test-quality claim is established. The constructed framework fixtures establish only their measured execution and context boundaries, not functional completion of arbitrary requests.
