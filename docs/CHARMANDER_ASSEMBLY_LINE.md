...
* independently accept one retained semantic leaf, remove one irrelevant list leaf, or return one complete corrected value for that same leaf;
...
6. A separately routed skeptical reviewer receives the same request and facts plus one code-selected retained leaf. It returns only `accept`, `remove` when code permits list removal, or one complete replacement value for that same leaf. It cannot choose another target or emit a finding, instruction, plan, path, tool, task identity, or state transition. Code binds the decision to the request digest and current value hash, performs the exact string/list splice itself, rejects byte-identical candidates and repeated states explicitly, and repeats independent review only after a real state change. Code then assigns requirement identities and binds each statement to the immutable request digest. There is no one-shot quote gate, compatibility route, or fallback requirements implementation.
...
8. For each accepted requirement, the constructive job-specification station receives the typed surface, reviewed product context, all accepted requirement statements, and one focused requirement. It returns only one concrete objective, one through four required behaviors, and one through four observable acceptance criteria. A separately routed skeptical model reviews exactly one code-enumerated current leaf. It returns only accept, remove, or one complete replacement value for that same leaf; it cannot return an instruction, plan, path, tool, task identity, or other state. Code couples the permitted decision and exact evidence value to the response schema and attaches a SHA-256 binding of the observed field. Invalid evidence never reaches a state change. Code performs the exact hash-bound removal or string replacement itself, persists the resulting specification, and independently reviews the changed state. A byte-identical replacement is rejected as an invalid reviewer candidate and re-reviewed against the same authoritative state; it never enters response/schema correction. Review continues while states are new and stops only on acceptance, an exact repeated invalid candidate, lost authority, provider failure, cancellation, or a real resource limit. Code then assigns task identity and source order and freezes the complete workload under one content hash. Models cannot return task identity, dependencies, order, tools, paths, completion state, or a whole plan. Derived build decisions are labeled separately from exact user authority and may not invent capabilities, quantities, timing, defaults, or constraints absent from that authority.
...
A decoded semantic response is retained in code and never replayed to another semantic model. Application intent and job-specification correction are reviewer-owned and leaf-scoped. For job specifications, code enumerates the whole objective and each whole current list item, binds the observed target value before any state change, and receives only accept, remove, or the complete replacement leaf from review. Code performs the exact reviewed removal or replacement itself. A byte-identical replacement is an invalid candidate, not malformed JSON and not a response-correction job. Repository requirement extraction remains a separate existing-repository boundary and must not be used as an application-intent fallback. Other semantic stations that permit structural correction receive only the exact validation failure and one schema-bound mutable field, never the full prior response or unrelated accepted fields.
