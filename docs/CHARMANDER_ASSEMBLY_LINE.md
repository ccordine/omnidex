...
8. For each accepted requirement, one job-specification station receives the typed surface, reviewed product context, all accepted requirement statements, and one focused requirement. It returns only one concrete objective, one through four required behaviors, and one through four observable acceptance criteria. A separately routed review model sees the same user authority plus that derived candidate and may accept it or name exactly one derived field, one bounded diagnostic finding, and one verbatim excerpt from that current field; it cannot contribute replacement prose or new requirements. Code proves that the excerpt occurs in the named field and attaches a SHA-256 binding of that exact observed value. Invalid evidence never reaches repair: code re-reviews the unchanged candidate with the exact validation failure and rejects repeated invalid evidence as a cycle. A repair call receives only focused user authority, that finding and validated excerpt, the named field, and the current value of that field. Code revalidates both the field hash and whole retained-authority binding before dispatch, applies the one-field replacement to retained state, requires another independent review, and preserves every new canonical specification state. Diagnostic excerpts ground current state; they do not reinstate substring or quote-allocation tests as user-authority gates. Review and repair have no numeric correctness limit: they continue while states are new and stop only on acceptance, an exact no-op or repeated state/cycle, lost authority, provider failure, cancellation, or a real resource limit. Code then assigns task identity and source order and freezes the complete workload under one content hash. Models cannot return task identity, dependencies, order, tools, paths, completion state, or a whole plan. Derived build decisions are labeled separately from exact user authority and may not invent capabilities, quantities, timing, defaults, or constraints absent from that authority.
...
A decoded semantic response is retained in code and never replayed to the correction model. Application intent and job-specification correction are reviewer-owned and leaf-scoped: the reviewer supplies one target and finding, and a job-specification reviewer must also cite one exact excerpt from the current target. Code rejects an absent excerpt, binds the observed target value, and re-runs review on unchanged state before any actuator is allowed to run. The correction schema permits only the validated target, and code merges the replacement into retained state. A job-specification correction sees only the focused requirement, current target value, finding, and validated evidence—not sibling requirements or other retained semantic fields. Repository requirement extraction remains a separate existing-repository boundary and must not be used as an application-intent fallback. Other semantic stations that permit structural correction receive only the exact validation failure and one schema-bound mutable field, never the full prior response or unrelated accepted fields.
