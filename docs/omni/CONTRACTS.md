# Omnidex Contracts
## Specialist Role Contracts For Deterministic Agentic Execution

Prepared: 2026-05-07 (US/Eastern)  
Contract Version: 1.0

## 1) Scope
This document defines strict machine-validated contracts for Omnidex specialist roles.

The purpose of these contracts is to let any supported model, or chain of models, participate in agentic workflows without giving stochastic text direct control over tools, memory, or side effects. Models propose bounded structured outputs; deterministic code validates and executes.

Initial priority roles:
...
### 4.1 Purpose
Produce one deterministic, reviewable migration bundle (PostgreSQL-style up/down discipline) from a bounded change request.
